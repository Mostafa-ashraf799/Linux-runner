...
class DistroSetupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (DistroStorageManager(this).isInstalled(distro)) {
            ...
            // AND past any download/copy step. Calling ensureInstalled()
            // here (rather than beginSetup with a Download source) matters:
            // ensureInstalled() itself checks isInstalled() first and
            // returns immediately with Done, so no network request or file
            // picker is triggered at all. The previous version of this code
            // called beginSetup(RootfsSource.Download) directly, which
            // skipped that check and always attempted a network download —
            // this is what caused "بيانات تنزيل تلقائي" to run even when
            // the user had chosen "اختيار ملف" and the distro was already
            // marked installed from an earlier attempt.
            binding.sourceChoiceLayout.visibility = View.GONE
            binding.setupProgressBar.visibility = View.VISIBLE
            lifecycleScope.launch {
                engine.ensureInstalled { progress -> runOnUiThread { render(progress) } }
            }
            return
        }
