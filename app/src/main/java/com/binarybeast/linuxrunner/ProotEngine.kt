...
class ProotEngine(
    ...
    private fun downloadRootfs(distro: Distro, onPercent: (Int) -> Unit) {
        ...
        // Cast explicitly to HttpURLConnection and call connect() before
        // reading any header — some servers (Ubuntu's Canonical mirror
        // included) respond with Transfer-Encoding: chunked instead of a
        // plain Content-Length, and reading contentLengthLong on an
        // un-connected generic URLConnection could previously surface a
        // malformed header value (seen as a NumberFormatException on
        // "time=..." in production) instead of the safe "-1 = unknown
        // size" fallback this code already intended.
        ...
        val totalBytes = try {
            ...
            -1L // Unknown size (e.g. chunked transfer-encoding) — progress bar just won't show a percentage.
        }
        var downloaded = 0L
